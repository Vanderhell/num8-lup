...
{ uint32_t len=HDR+8u+CRC; if(out_len) *out_len=0u; if(!out||!p||!out_len||cap<len) return 0; common(out,NUM8LORA_OP_MSG_BEACON,sid,0u,seq); st32(&out[8],p->stream_id); st32(&out[12],p->latest_op_id); finish_crc(out,len); *out_len=len; return 1; }
...
{ uint32_t len=HDR+8u+CRC; if(out_len) *out_len=0u; if(!out||!p||!out_len||cap<len) return 0; common(out,NUM8LORA_OP_MSG_REQUEST,sid,rid,seq); st32(&out[8],p->stream_id); st32(&out[12],p->next_needed_op_id); finish_crc(out,len); *out_len=len; return 1; }
...
{ uint32_t len=HDR+16u+CRC; if(out_len) *out_len=0u; if(!out||!p||!out_len||cap<len||p->reserved0!=0u||p->reserved1!=0u) return 0; common(out,NUM8LORA_OP_MSG_DATA,sid,rid,seq); st32(&out[8],p->stream_id); st32(&out[12],p->op_id); out[16]=p->op_type; out[17]=0u; st16(&out[18],0u); st32(&out[20],p->value); finish_crc(out,len); *out_len=len; return 1; }
...
{ uint32_t len=HDR+8u+CRC; if(out_len) *out_len=0u; if(!out||!p||!out_len||cap<len) return 0; common(out,NUM8LORA_OP_MSG_ACK,sid,rid,seq); st32(&out[8],p->stream_id); st32(&out[12],p->ack_op_id); finish_crc(out,len); *out_len=len; return 1; }
...
{ uint32_t len=HDR+12u+CRC; if(out_len) *out_len=0u; if(!out||!p||!out_len||cap<len) return 0; common(out,NUM8LORA_OP_MSG_NACK,sid,rid,seq); st32(&out[8],p->stream_id); st16(&out[12],p->error_code); st16(&out[14],p->detail); st32(&out[16],p->expected_next_op_id); finish_crc(out,len); *out_len=len; return 1; }

int num8lora_op_decode_common_header(const uint8_t* b, uint32_t len, num8lora_op_common_header_t* o)
{ if(o) { o->protocol_version=0u; o->msg_type=0u; o->sender_id=0u; o->receiver_id=0u; o->seq=0u; } if(!b||!o||len<HDR) return 0; o->protocol_version=b[0]; o->msg_type=b[1]; o->sender_id=ld16(&b[2]); o->receiver_id=ld16(&b[4]); o->seq=ld16(&b[6]); return 1; }
...
{ if(p) { p->stream_id=0u; p->latest_op_id=0u; } if(!b||!h||!p||len!=18u||!prep(b,len,NUM8LORA_OP_MSG_BEACON,h)||h->receiver_id!=0u) return 0; p->stream_id=ld32(&b[8]); p->latest_op_id=ld32(&b[12]); return 1; }
...
{ if(p) { p->stream_id=0u; p->next_needed_op_id=0u; } if(!b||!h||!p||len!=18u||!prep(b,len,NUM8LORA_OP_MSG_REQUEST,h)) return 0; p->stream_id=ld32(&b[8]); p->next_needed_op_id=ld32(&b[12]); return 1; }
int num8lora_op_decode_data(const uint8_t* b, uint32_t len, num8lora_op_common_header_t* h, num8lora_op_data_payload_t* p)
{ if(p) { p->stream_id=0u; p->op_id=0u; p->op_type=0u; p->reserved0=0u; p->reserved1=0u; p->value=0u; } if(!b||!h||!p||len!=26u||!prep(b,len,NUM8LORA_OP_MSG_DATA,h)) return 0; p->stream_id=ld32(&b[8]); p->op_id=ld32(&b[12]); p->op_type=b[16]; p->reserved0=b[17]; p->reserved1=ld16(&b[18]); p->value=ld32(&b[20]); return p->reserved0==0u&&p->reserved1==0u; }
int num8lora_op_decode_ack(const uint8_t* b, uint32_t len, num8lora_op_common_header_t* h, num8lora_op_ack_payload_t* p)
{ if(p) { p->stream_id=0u; p->ack_op_id=0u; } if(!b||!h||!p||len!=18u||!prep(b,len,NUM8LORA_OP_MSG_ACK,h)) return 0; p->stream_id=ld32(&b[8]); p->ack_op_id=ld32(&b[12]); return 1; }
int num8lora_op_decode_nack(const uint8_t* b, uint32_t len, num8lora_op_common_header_t* h, num8lora_op_nack_payload_t* p)
{ if(p) { p->stream_id=0u; p->error_code=0u; p->detail=0u; p->expected_next_op_id=0u; } if(!b||!h||!p||len!=22u||!prep(b,len,NUM8LORA_OP_MSG_NACK,h)) return 0; p->stream_id=ld32(&b[8]); p->error_code=ld16(&b[12]); p->detail=ld16(&b[14]); p->expected_next_op_id=ld32(&b[16]); return 1; }
